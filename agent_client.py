...
def fetch_price(endpoint: str) -> int:
    """Fetch current price from /price/{endpoint}"""
...
def request_invoice(endpoint: str, payload: dict):
    """Send request and extract invoice + token if 402"""
    resp = requests.post(f"{API_BASE}/{endpoint}", json=payload, timeout=15)

    if resp.status_code == 200:
        # Already paid or free endpoint
...
def pay_with_lnd(bolt11: str) -> str:
    if not LNDClient:
        raise RuntimeError("lndgrpc is not installed. Run: pip install lndgrpc")
    if not LND_DIR:
        raise RuntimeError("LND_DIR environment variable not set")

    print("Paying invoice via LND...")
    lnd = LNDClient(LND_DIR)
    response = lnd.send_payment_sync(payment_request=bolt11)
    return response.payment_preimage.hex()   # return as hex string


def pay_with_cln(bolt11: str) -> str:
    if not LightningRpc:
        raise RuntimeError("pyln-client is not installed. Run: pip install pyln-client")
    if not CLN_RPC_PATH:
        raise RuntimeError("CLN_RPC_PATH environment variable not set")

    print("Paying invoice via Core Lightning...")
...
def main():
    ...
    if args.endpoint == "decision":
        if not all([args.goal, args.context, args.question]):
            print("Error: For --endpoint=decision you must provide --goal, --context and --question")
            sys.exit(1)
        payload = {
            "goal": args.goal,
        ...
        if not args.question:
            print("Error: For --endpoint=reason you must provide --question")
            sys.exit(1)
    ...
    try:
        ...
        if already_paid:
            print("Already paid or free response received:")
    ...
    try:
        if LND_DIR and LNDClient:
            ...
            print("Error: No Lightning node configured. Set LND_DIR or CLN_RPC_PATH")
            sys.exit(1)
    except Exception as e:
        print(f"Payment failed: {e}")
        sys.exit(1)

    print(f"Payment successful! Preimage: {preimage}")

    # Step 4: Retry request with L402 header
