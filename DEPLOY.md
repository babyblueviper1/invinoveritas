# DEPLOY.md — invinoveritas v1.6.0 Deployment Guide
...
The v1.6.0 upgrade moves storage from `/opt/render/...` to `/root/invinoveritas/data/`:
...
### New v1.6.0 endpoints smoke test
```bash
# Marketplace
...
git commit -m "feat: v1.6.0 — marketplace, orchestration, analytics, NWC defaults
...
python -m build
# Creates: dist/invinoveritas-1.6.0-py3-none-any.whl
#          dist/invinoveritas-1.6.0.tar.gz
...
- [ ] `curl https://api.babyblueviper.com/health` returns `"version": "1.6.0"`
- [ ] `curl https://api.babyblueviper.com/offers/list` returns `{"offers": [], ...}`
- [ ] `pip install --upgrade invinoveritas && python -c "import invinoveritas; print(invinoveritas.__version__)"` prints `1.6.0`
- [ ] GitHub shows the v1.6.0 commit
- [ ] Create GitHub release tag: `git tag v1.6.0 && git push origin v1.6.0`
...
# Push to GitHub
git add -A && git commit -m "v1.6.0" && git push origin main
```
