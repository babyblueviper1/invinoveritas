# DEPLOY.md — invinoveritas v1.1.0 Deployment Guide
...
The v1.1.0 upgrade moves storage from `/opt/render/...` to `/root/invinoveritas/data/`:
...
### New v1.1.0 endpoints smoke test
```bash
# Marketplace
...
python -m build
# Creates: dist/invinoveritas-1.1.0-py3-none-any.whl
#          dist/invinoveritas-1.1.0.tar.gz
...
- [ ] `curl https://api.babyblueviper.com/health` returns `"version": "1.1.0"`
- [ ] `curl https://api.babyblueviper.com/offers/list` returns `{"offers": [], ...}`
- [ ] `pip install --upgrade invinoveritas && python -c "import invinoveritas; print(invinoveritas.__version__)"` prints `1.1.0`
- [ ] GitHub shows the v1.1.0 commit
- [ ] Create GitHub release tag: `git tag v1.1.0 && git push origin v1.1.0`
...
# Push to GitHub
git add -A && git commit -m "v1.1.0" && git push origin main
```
